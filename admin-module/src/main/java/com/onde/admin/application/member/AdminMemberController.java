...
public class AdminMemberController {
    ...
    @PostMapping("/members/{id}/reveal")
    @PreAuthorize("hasRole('SUPER_ADMIN')")
    public ResponseEntity<ApiResponse<AdminMemberRevealResponse>> revealMember(
        ...
        AdminMemberRevealResponse response = AdminMemberRevealResponse.builder()
                ...
                .name(member.getName())
                .build();
        return ResponseEntity.ok(ApiResponse.success(response, "회원 원문 조회 성공"));
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> updateRole(@PathVariable Long id,
        ...
        Map<String, Object> data = new LinkedHashMap<>();
        data.put("memberId", PersonalDataMasker.maskNumericId(id));
        data.put("email", PersonalDataMasker.maskEmail(member.getEmail()));
    ...
    private Long getAdminIdFromPrincipal(Principal principal) {
        return adminMemberIdentitySupport.requireMemberId(principal);
    }
}
