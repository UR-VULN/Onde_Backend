...
public class PaymentController {
    ...
    /**
     * PG사 결제창을 띄우기 전, 백엔드에 결제 요청 정보를 사전 등록하고 금액 정합성을 검증합니다.
     * 프론트엔드는 이 API의 응답으로 받은 merchantUid를 사용하여 포트원 결제창을 호출해야 합니다.
     *
     * @param userId 로그인한 회원의 식별자 (임시로 X-User-Id 헤더에서 추출)
    ...
    public ResponseEntity<ApiResponse<PaymentPrepareResponse>> preparePayment(
        ...
        return ResponseEntity.ok(ApiResponse.success(result, "결제 사전 검증 완료. merchantUid로 PG 결제를 진행하세요."));
    }

    /**
     * PG사 결제 완료 후, 실제 결제 금액이 사전에 약속된 금액과 일치하는지 최종 검증(사후 검증)을 수행하고
     * 결제 내역을 저장 및 유저 마일리지를 차감합니다.
     *
     * @param userId 로그인한 회원의 식별자 (임시로 X-User-Id 헤더에서 추출)
     * @param body   포트원의 impUid 및 merchantUid가 포함된 요청 객체
